...
def init():
    assert sys.version_info >= (3, 3), "Must be run in Python 3.3 or later. You are running {}".format(sys.version)
    parser = argparse.ArgumentParser()
    parser.add_argument('--save_dir', type=str, default='chatbotrnn/models/reddit', help='model directory to store checkpointed models')
    parser.add_argument('-n', type=int, default=100, help='number of characters to sample')
    parser.add_argument('--prime', type=str, default=' ', help='prime text')
    parser.add_argument('--beam_width', type=int, default=2, help='Width of the beam for beam search, default 2')
    parser.add_argument('--temperature', type=float, default=1.0, help='sampling temperature'
                       '(lower is more conservative, default is 1.0, which is neutral)')
    parser.add_argument('--topn', type=int, default=-1, help='at each step, choose from only this many most likely characters;'
    ...
    return net, sess, chars, vocab, args
...
def _get_response(user_input, net, sess, chars, vocab, args):
    res = ''
    ...
    if not user_command_entered:
        ...
        for i, char_token in enumerate(computer_response_generator):
            ...
            res += possibly_escaped_char(out_chars)
...
net, sess, chars, vocab, args = init()

def get_response(msg):
    return _get_response(msg, net, sess, chars, vocab, args)
